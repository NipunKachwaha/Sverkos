...
import '@/components/ui/ParticleText/ParticleText.css';
...
const ParticleText = ({
  ...
  particleSize = 2,
  density = 4,
  color = '#ffffff', // This prop now has no effect, color is overridden to white below.
  ...
  useEffect(() => {
    ...
    const drawParticle = (particle: Particle): void => {
      ...
      ctx.fillStyle = '#fff'; // Always white dot
      // If you want to use white from the particle's color property, use:
      // ctx.fillStyle = '#fff';
    ...
    const sampleText = async (): Promise<void> => {
      ...
      const step = Math.max(1, Math.floor(density / 2)); // INCREASE PARTICLE DENSITY (smaller step, more particles)
      ...
      // INCREASE PARTICLE COUNT: allow more maxParticles, and higher upper limit
      const maxParticles = Math.max(2500, Math.min(20000, Math.floor((width * height) / 30)));
      const stride = Math.max(1, Math.ceil(targets.length / maxParticles));
      // All dots are white
      // Override all particle color to white, using rgbToCss if you wish (same as "#fff")
      const selected = targets.filter((_, index) => index % stride === 0);

      particles = selected.map((target, index) => {
        const seed = ((index * 9301 + 49297) % 233280) / 233280;
        const depth = 0.45 + (((index * 233 + 97) % 1000) / 1000) * 0.9;
        // Always use pure white color for the particle
