...
vec3 fourColorGradient(vec2 uv) {
  // bilinear interpolation with the four corners:
  // uColor1 = bottom left (0,0)
  // uColor2 = bottom right (1,0)
  // uColor3 = top right (1,1)
  // uColor4 = top left (0,1)
  vec3 bottom = mix(uColor1, uColor2, uv.x);
  vec3 top = mix(uColor4, uColor3, uv.x);
...
void main() {
  ...
  if (uHasImage > 0.5) {
    ...
    base = fourColorGradient(clamp(vUv, 0.0, 1.0));
  }
...
// Defaults for four colors
const DEFAULT_COLOR1 = '#5227FF'; // bottom-left
const DEFAULT_COLOR2 = '#B19EEF'; // bottom-right
const DEFAULT_COLOR3 = '#FF904E'; // top-right (new)
const DEFAULT_COLOR4 = '#23E2EA'; // top-left (new)

// You may wish to update the defaults as needed above!
...
const ElasticMesh = ({
  ...
  useEffect(() => {
    ...
    function frame(now: number) {
      ...
      program.uniforms.uColor3.value = hexToRgb(p.color3);
      program.uniforms.uColor4.value = hexToRgb(p.color4);
